...
운영자가 신규 K-pop 상품을 처음 등록하는 화면. 4단계 위저드:
1. **카테고리 + 기본 정보** — category_id (트리에서 선택), global_item_name (운영자가 입력 시 `[PRE ORDER]` 자동 prefix), description, brand, original_price, normal_stock, condition
2. **이미지 & 미디어** — 이미지 업로드(`media_space/upload_image`) → image_id_list 자동 채움
3. **옵션/변형 (variants)** — 옵션 있으면 model SKU + weight + 옵션값. 없으면 skip
4. **region publish** — `get_publishable_shop` 으로 candidate shops 표시 → 사용자가 region 6개 체크박스 → `create_publish_task` 호출 + task_id 받음 → `get_publish_task_result` polling (5s 간격, 60s timeout)
5. **완료 시점 자동 수행**
   - `products` row INSERT: lifecycle_state='pre_order', cost_price, weight, image_url, shopee_item_id
   - `product_shopee_listings` rows INSERT (region별): days_to_ship=50, title_state='PRE_ORDER', last_pushed_name, last_pushed_at
   - 모든 region 의 days_to_ship 을 50 으로 push (`update_shop_days_to_ship`)
   - PRE ORDER 분류 완료
