...
const ebayPublishGroupBuilderFn = new Function(
  ...
  () => false,
  (row) => row.option_name || '',
  (row, fallback = '') => (Array.isArray(row?.variation_option_names) && row.variation_option_names.length ? row.variation_option_names.join(' / ') : String(row?.option_name || fallback || '').trim()),
  () => ({ tierNames: [], twoAxis: false, optionNamesForRow: () => [] }),
  (rows) => rows[0]?.sku || '',
...
const platformOpenActionFn = new Function(
  ...
  'platformCanUseDispatcher',
  'platformBatchSelectionMode',
  'platformStartBatchRegistration',
  'platformGuardBatchInterruption',
  'platformGroupProductIds',
  'platformOpenEditFlow',
  'PLATFORM_LABELS',
  'showToast',
  'renderPlatformWorkbench',
...

{
  ...
  const openAction = platformOpenActionFn(
    ...
    () => {},
    () => false,
    (group) => (group.rows || []).map((row) => row.id).filter(Boolean),
    async () => {},
    { ebay: 'eBay' },
    (message, kind) => calls.toasts.push({ message, kind }),
    (platform) => calls.renders.push(platform),
