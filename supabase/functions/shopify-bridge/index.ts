...
function normalizeIdList(value: unknown): string[] {
  ...
  const text = norm(value);
  if (!text) return [];
  return uniq(text.split(',').map((entry) => entry.trim()));
}
...
async function shopifyRepriceCandidateRows(shop: any, policy: any, limit: number, filters: any = {}) {
  const masterProductIds = normalizeIdList(filters.masterProductIds || filters.master_product_ids);
  const listingIds = normalizeIdList(filters.listingIds || filters.listing_ids);
  let query = supabase
  ...
  if (masterProductIds.length) query = query.in('master_product_id', masterProductIds);
...
async function handleRepriceProducts(req: Request): Promise<Response> {
  ...
  const masterProductIds = normalizeIdList(body?.master_product_ids || body?.masterProductIds);
  const listingIds = normalizeIdList(body?.listing_ids || body?.listingIds);
  ...
  const { rows, skipped } = await shopifyRepriceCandidateRows(shop, policy, limit, { masterProductIds, listingIds });
  ...
  return jsonResp({
    ...
    filters: { master_product_ids: masterProductIds, listing_ids: listingIds },
    counts: {
...
async function handleArchiveProduct(req: Request): Promise<Response> {
  ...
  const masterProductIds = normalizeIdList(body.product_ids || body.master_product_ids || body.masterProductIds);
  if (body.dry_run === true) {
    return jsonResp({
      ...
      filters: { master_product_ids: masterProductIds },
    });
  }
  ...
  let archivedListings: any[] = [];
  let listingUpdateError: string | null = null;
  try {
    let query = supabase
      .from('platform_listings')
      .update({
      ...
      .eq('platform', 'shopify')
      .eq('platform_item_id', productId)
      .is('deleted_at', null);
    if (masterProductIds.length) query = query.in('master_product_id', masterProductIds);
    const { data, error } = await query.select('id,master_product_id,listing_status,mapping_status');
    if (error) listingUpdateError = error.message;
    else archivedListings = Array.isArray(data) ? data : [];
  } catch (e) {
    listingUpdateError = e?.message || String(e);
  }
  return jsonResp({
    ok: true,
    shop_domain: shop.shop_domain,
    ...
    listing_update_error: listingUpdateError,
    raw: archived.raw,
  }, listingUpdateError ? 207 : 200);
}
