-- Allow WMS preview cards to attach Staronemall enrichment after initial
-- read-only WMS staging. This never writes back to the WMS inventory project.

drop function if exists public.update_wms_source_observed(uuid, jsonb);
...
begin
  ...
  update public.source_records
     set observed_values = v_next,
         confidence = greatest(coalesce(confidence, 0), 95),
         fetched_at = now(),
         status = case
           when status in ('rejected', 'superseded') then 'pending_review'
           else status
         end
   where id = p_source_record_id
   returning id, public.source_records.status, public.source_records.observed_values
