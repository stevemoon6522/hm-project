-- Track whether a master product should use album or goods category defaults.
-- Existing WMS and StarOneMall-created products remain album by default.

alter table public.products
  add column if not exists product_kind text not null default 'album';
...
alter table public.products
  ...
  check (product_kind in ('album', 'goods'));

comment on column public.products.product_kind is
  'Master product type for platform category defaults. album = existing fixed album categories; goods = operator-selected goods categories.';
...
begin
  ...
  v_product_kind := lower(nullif(btrim(coalesce(v_payload ->> 'product_kind', 'album')), ''));
  if v_product_kind is null then
    v_product_kind := 'album';
  end if;
  if v_product_kind not in ('album', 'goods') then
    raise exception 'product_kind_invalid' using errcode = 'P0001';
  end if;
